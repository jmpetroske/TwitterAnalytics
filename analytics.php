<?php
	...
	$count = 0;
	$timestamps = array();
//	print_r($connection->get("statuses/user_timeline", $query));
	do {
        ...
        foreach($results as $status){
			$data = new stdClass();
			$data->time = strtotime($status->created_at);
			if (property_exists($status, "retweeted_status")) {
				$data->retweets = 0;
			} else {
				$data->retweets = $status->retweet_count;
			}
            $timestamps[] = $data;
			if ($status->id < $minID){
				$minID = $status->id;
	...
	} while (/*$count < $tweetMultiplier*/0==1); 
